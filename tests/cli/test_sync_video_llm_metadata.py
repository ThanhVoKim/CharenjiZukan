...
========================================
Test Layer 1 cho helper LLM metadata trong cli/sync_video.py.

Cấu trúc layers:
  Layer 1 — Unit Tests          (raw text builder, path policy, override, fail policy)
...
from cli.sync_video_llm_metadata import (  # noqa: E402
    apply_llm_metadata_override,
    build_llm_metadata_input_text,
...
class TestLayer1_LLMMetadataHelpers:
    def test_build_input_text_is_flat_raw_text(self):
        segments = [
        ...
        assert build_llm_metadata_input_text(segments) == "First line continues Second block"
    ...
    def test_execute_writes_debug_input_and_metadata_output(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr("cli.llm_task._create_task_provider", fake_create_task_provider)
        ...
        assert (tmp_path / "content" / "a" / "b_metadata_input.txt").read_text(encoding="utf-8") == "Alpha Beta Gamma"
    ...
    def test_warn_fail_policy_swallows_metadata_errors(self, monkeypatch, tmp_path: Path):
        ...
        monkeypatch.setattr("cli.sync_video_llm_metadata.execute_llm_metadata_task", boom)

        result = run_llm_metadata_task(
            subtitle_segments=[{"text": "raw subtitle text"}],
