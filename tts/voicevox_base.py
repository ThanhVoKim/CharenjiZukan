...
class VoicevoxRestTTSEngine(BaseTTSEngine):
    """
    Shared async engine gọi REST API Voicevox (dùng chung cho Voicevox Nemo và Voicevox chính thức).

    Cả hai engine đều dùng endpoint /audio_query và /synthesis với cùng cấu trúc
    request/response. Sự khác biệt chỉ nằm ở defaults (port, speaker, concurrency, scale).
    """

    def __init__(
        self,
        queue_tts: List[Dict[str, Any]],
        engine_label: str = "VoicevoxRestTTS",
        voice_id: int = 10008,
        host: str = "127.0.0.1",
        port: int = 50121,
        speed_scale: float = 1,
    ...
    def _apply_query_parameters(self, query_data: Dict[str, Any]) -> None:
        """Ghi đè tham số synthesis vào query_data (mutate in-place)."""
        ...
        async with semaphore:
            for attempt in range(self.max_retries):
                try:
                    ...
                    self._apply_query_parameters(query_data)
                    ...
                    print(
                        f"[{index}] ⚠️ Lỗi (Lần thử {attempt + 1}/{self.max_retries}): {e}"
                    )
                    if attempt < self.max_retries - 1:
                        await asyncio.sleep(1)  # Nghỉ 1 giây trước khi thử lại
                    else:
                        print(
                            f"❌ Thất bại hoàn toàn câu {index} sau {self.max_retries} lần thử."
                        )
    ...
    def run(self) -> Dict[str, int]:
        """Entry point đồng bộ."""
        print(
            f"[{self.engine_label}] Bắt đầu {len(self.queue_tts)} dòng | voice_id={self.voice_id}"
        )
