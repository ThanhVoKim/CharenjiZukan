...
def recalculate_segments(
    ...
    """Remap timestamp của list segment sang timeline mới, trả về list segment dict.

    Tách từ `recalculate_srt` để dùng lại ở các nơi cần segment dict trong bộ nhớ
    (vd gộp với forced-alignment per-clip) thay vì ghi thẳng ra file.
...
def recalculate_srt(
    ...
    Tránh subtitle biến mất sớm trong khi audio vẫn đang phát.
    """
    new_segments = recalculate_segments(
        segments, timeline,
        is_tts_track=is_tts_track,
        max_chars=max_chars,
        fps_float=fps_float,
    )
